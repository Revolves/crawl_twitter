...
def create_file(_path):
    ...
    ip=ip.split('.')[2:4]
    save_file = open(_path+'\\twitter_'+datetime.datetime.now().strftime("%Y%m%d%H%M%S%f")[0:17]+'_'+ip[0]+'_'+ip[1], 'w',
...
def first_getTweet(api,datapath, first, count, user_count, namelist_file):
